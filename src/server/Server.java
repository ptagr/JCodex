...
public class Server implements Runnable {
	...
	LinkedHashMap<Long, Set<CODEXServerMessage>> messageCache = new AccessOrderCache<Long, Set<CODEXServerMessage>>(
			50);
...
//							shares[i++] = frra.getDecryptedShare();
					...
					BigInteger blindedSecret = stkm.thresholdDecrypt(
							correctCipher, shares);
					...
					if (messageCache.get(csm2.getNonce()) == null) {
						// Handle this later
						// Someone deleted the messages
						// Panic
					} else {
						Set<CODEXServerMessage> csmSet2 = messageCache.get(csm2
								.getNonce());

						for (CODEXServerMessage csmtemp : csmSet2) {
							if (csmtemp
									.getType()
									.equals(CODEXServerMessageType.SIGNED_READ_RESPONSE)) {
						...
						BigInteger digitalSigRes = stkm.thresholdSign(brrBytes,
								sigs);
