...
public class Dealer implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1396276549306683275L;
	 ...
	 */
	public BigInteger generateKeys(final int k, final int l) {
...
//		pr = (p.subtract(ThreshUtil.ONE));
//		qr = (q.subtract(ThreshUtil.ONE));
		// m = pr*qr
		m = pr.multiply(qr);
		...
		// Otherwise pick a prime bigger then groupSize
			...
			e = new BigInteger(groupSize.bitLength() + 1, 80,
	...
	private KeyShare[] generateKeyShares(final BigInteger d,
			final BigInteger m, final int k, final int l, final BigInteger n) {
		BigInteger[] secrets;
		BigInteger rand;
		int randbits;
		...
		for (int i = 0; i < l; i++){
			s[i] = new KeyShare(i + 1, secrets[i]);
	...
	private BigInteger generateVerifiers(
		...
		BigInteger n = gk.getModulus();

		v = new BigInteger[secrets.length];

		// rand is an element of Q*n (squares of relative primes mod n)
		// MOving it out of the secrets loop below to generate only one group
		// verifier
		while (true) {
		...
		for (int i = 0; i < secrets.length; i++) {
			...
			v[i] = rand.modPow(secrets[i].getSecret(), n);
			secrets[i].setVerifier(v[i]);
		}

		// for (final KeyShare element : secrets) {
		// // Check if rand is equal for everyone
		// BigInteger gv = element.getGroupVerifier();
		// System.out.println(gv);
		//
		// }
		
		//Set the verifiers in group key
		gk.setV(rand);
		gk.setVi(v);
